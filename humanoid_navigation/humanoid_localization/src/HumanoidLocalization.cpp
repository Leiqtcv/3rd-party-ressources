      ...
      m_distLinear(0.1), m_distAngular(0.1)
{
    ...
    //LC: moved before reset() to enable imu value integration in first global initialization TODO: not working yet
    if (m_useIMU)
        m_imuSub = m_nh.subscribe("imu", 10, &HumanoidLocalization::imuCallback, this);
    if (m_useTimer)
    {
    ...
    //LC: added to get sensor messages
    ros::spinOnce();

    //LC: moved to end to use sensor callbacks, originally before global localization service instantiation
...
bool HumanoidLocalization::isAboveMotionThreshold(const tf::Pose& odomPose){
    ...
    if(odomTransform.getOrigin().length() >= m_observationThresholdTrans){
        ROS_INFO("length");
        return true;
    }
    if(std::abs(yaw) >= m_observationThresholdRot){
        ROS_INFO("yaw");
        return true;
    }
    return false;
    ...
    if (!(m_constrainMotionRP && m_constrainMotionZ)){
        ...
        if(imuMsgOk) {
            if (!m_motionModel->lookupLocalTransform(m_baseFootprintId, t, footprintToTorso)) {
                ...
                //        std::cout << footprintToTorso.getOrigin().x() << " " << footprintToTorso.getOrigin().y() << " " << footprintToTorso.getOrigin().z() << std::endl;
                //        ROS_INFO("angleX = %f  angleY = %f", angleX, angleY);
...
void HumanoidLocalization::pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& msg) {
    ...
    if (headYawRotationSinceScan>= m_observationThresholdHeadYawRot || headPitchRotationSinceScan >= m_observationThresholdHeadPitchRot) {
        ROS_INFO("Motion threshold exceeded");
...
void HumanoidLocalization::initPoseCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg){
    ...
    if (std::abs(pose.getOrigin().getZ()) < 0.01){
        m_mapModel->getHeightlist(pose.getOrigin().getX(), pose.getOrigin().getY(), 0.0, 0.6, heights);
        if (heights.size() == 0){
