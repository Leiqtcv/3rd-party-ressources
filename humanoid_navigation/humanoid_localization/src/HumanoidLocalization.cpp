...
m_maxHeight(2.0)                                                                                // added by LC
{
  ...
  //moved before reset() to enable imu value integration in first global initialization TODO: not working yet
  if (m_useIMU)
    m_imuSub = m_nh.subscribe("imu", 50, &HumanoidLocalization::imuCallback, this);
  if (m_useTimer)
  {
...
void HumanoidLocalization::initGlobal(){
  ...
  m_mapModel->initGlobal(m_particles, z, roll, pitch, m_initNoiseStd, m_rngUniform, m_rngNormal, m_maxHeight);
