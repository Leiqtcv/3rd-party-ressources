...
bool MotionModel::lookupOdomTransform(const ros::Time& t, tf::Transform& odomTransform) const{
  ...
  if (t <= m_lastOdomPose.stamp_ && (m_lastOdomPose.stamp_ - t).toSec()*1000 > 20){     //modified by LC, only warn if last odom pose is older than 20 ms
    ROS_WARN("Looking up OdomTransform that is %f ms older than the last odomPose!",
             (m_lastOdomPose.stamp_ - t).toSec()*1000.0);                               //modified by LC, ms equals sec*1000 instead of sec/1000
  }
